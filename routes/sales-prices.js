// routes/sales-prices.js
// 掛載方式：app.use('/api/sales-prices', require('./routes/sales-prices')(pool, schemaName))
...
module.exports = (pool, schemaName) => {
  ...
  // ══════════════════════════════════════════════
  // 1. 查詢版本清單
  // GET /api/sales-prices/versions
  // ══════════════════════════════════════════════
  router.get('/versions',
    authenticateToken,
    requirePermission('sales:price:read'),
    async (req, res) => {
      try {
        const result = await pool.query(`
          ...
          FROM ${schemaName}.sales_price_versions v
          LEFT JOIN ${schemaName}.sales_price_rows r
  ...
  // ══════════════════════════════════════════════
  // 2. 查詢單一版本明細（價格列）
  // GET /api/sales-prices/versions/:versionCode/rows
  // ══════════════════════════════════════════════
    ...
    async (req, res) => {
      try {
        ...
        const vCheck = await pool.query(
          `SELECT version_code FROM ${schemaName}.sales_price_versions WHERE version_code = $1`,
          [versionCode]
        );
  ...
  // ══════════════════════════════════════════════
  // 3. 新增版本（系統自動產生版本號）
  // POST /api/sales-prices/versions
  // Body: { start_date, end_date, created_by? }
  // ══════════════════════════════════════════════
  router.post('/versions',
    authenticateToken,
    requirePermission('sales:price:create'),
    async (req, res) => {
      const client = await pool.connect();
      try {
        ...
        const insertResult = await client.query(`
          INSERT INTO ${schemaName}.sales_price_versions (
        ...
        if (copy_version_code) {
          ...
          if (srcCheck.rows.length > 0) {
            await client.query(`
              INSERT INTO ${schemaName}.sales_price_rows
        ...
        const countRes = await pool.query(
          `SELECT COUNT(*) AS cnt FROM ${schemaName}.sales_price_rows WHERE version_code = $1`,
          [versionCode]
        );
  ...
  router.put('/versions/:versionCode/rows',
    ...
    async (req, res) => {
      ...
      try {
        ...
        // 先刪除舊明細，再批次插入（覆寫策略）
        await client.query(
          `DELETE FROM ${schemaName}.sales_price_rows WHERE version_code = $1`,
          [versionCode]
        );
        ...
        await client.query(`
          UPDATE ${schemaName}.sales_price_versions
  ...
  // ══════════════════════════════════════════════
  // 5. 刪除版本（連同明細）
  // DELETE /api/sales-prices/versions/:versionCode
  // ══════════════════════════════════════════════
  router.delete('/versions/:versionCode',
    authenticateToken,
    requirePermission('sales:price:delete'),
    async (req, res) => {
      const client = await pool.connect();
      try {
        await client.query('BEGIN');
        ...
        await client.query(
          `DELETE FROM ${schemaName}.sales_price_versions WHERE version_code = $1`,
          [versionCode]
        );
  ...
  // GET /api/sales-prices/active
  // GET /api/sales-prices/active?product_code=TS0001
  // ══════════════════════════════════════════════
  router.get('/active',
    authenticateToken,
    requirePermission('sales:price:read'),
      ...
      try {
        ...
        let query = `
          SELECT
            ...
            v.start_date,
            v.end_date
          FROM ${schemaName}.sales_price_rows r
          JOIN ${schemaName}.sales_price_versions v ON r.version_code = v.version_code
