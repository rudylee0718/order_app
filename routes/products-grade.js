// routes/products-grade.js
// 掛載方式：app.use('/api/products-grade', require('./routes/products-grade')(pool, schemaName))
...
module.exports = (pool, schemaName) => {
  ...
  // ══════════════════════════════════════════════
  // 1. 查詢版本清單
  // GET /api/products-grade/versions
  // ══════════════════════════════════════════════
  router.get('/versions',
    authenticateToken,
    requirePermission('sales:price-level:read'),
    async (req, res) => {
      try {
        const result = await pool.query(`
          ...
          FROM ${schemaName}.products_grade_versions v
          LEFT JOIN ${schemaName}.products_grade_rows r
  ...
  // ══════════════════════════════════════════════
  // 2. 查詢單一版本明細（等級列）
  // GET /api/products-grade/versions/:versionCode/rows
  // ══════════════════════════════════════════════
    ...
    async (req, res) => {
      try {
        ...
        const vCheck = await pool.query(
          `SELECT version_code FROM ${schemaName}.products_grade_versions WHERE version_code = $1`,
          [versionCode]
        );
  ...
  // ══════════════════════════════════════════════
  // 3. 新增版本（系統自動產生版本號）
  // POST /api/products-grade/versions
  // Body: { start_date, end_date, created_by? }
  // ══════════════════════════════════════════════
  router.post('/versions',
    authenticateToken,
    requirePermission('sales:price-level:create'),
    async (req, res) => {
      const client = await pool.connect();
      try {
        ...
        const insertResult = await client.query(`
          INSERT INTO ${schemaName}.products_grade_versions (
        ...
        if (copy_version_code) {
          ...
          if (srcCheck.rows.length > 0) {
            await client.query(`
              INSERT INTO ${schemaName}.products_grade_rows
                (version_code, class_code, class_name, grade, discount,pmcode, remark, created_by, updated_by)
              SELECT $1, class_code, class_name, grade, discount,pmcode, remark, $2, $2
              FROM ${schemaName}.products_grade_rows
        ...
        const countRes = await pool.query(
          `SELECT COUNT(*) AS cnt FROM ${schemaName}.products_grade_rows WHERE version_code = $1`,
          [versionCode]
        );
  ...
  // ══════════════════════════════════════════════
  // 4. 批次儲存版本明細（覆寫）
  // PUT /api/products-grade/versions/:versionCode/rows
  // Body: { rows: [{ class_code, class_name, grade, discount,pmcode, remark }] }
  // ══════════════════════════════════════════════
  router.put('/versions/:versionCode/rows',
    authenticateToken,
    requirePermission('sales:price-level:update'),
    async (req, res) => {
      const client = await pool.connect();
      try {
        await client.query('BEGIN');
        ...
        if (!Array.isArray(rows) || rows.length === 0) {
          await client.query('ROLLBACK');
          return res.status(400).json({ status: 'Error', message: '請提供至少一筆分類資料' });
        }

        for (const row of rows) {
          if (!row.class_code || !row.grade || row.discount === undefined || row.discount === null || !row.pmcode) {
            await client.query('ROLLBACK');
            return res.status(400).json({
              status: 'Error',
              message: `分類、等級、折扣、PMCODE為必填（分類：${row.class_code || '未填'}）`
            });
          }
          if (isNaN(Number(row.discount)) || Number(row.discount) < 0) {
            await client.query('ROLLBACK');
            return res.status(400).json({
              status: 'Error',
              message: `分類 ${row.class_code} 的價格格式不正確`
            });
          }
        }
        ...
        // 先刪除舊明細，再批次插入（覆寫策略）
        await client.query(
          `DELETE FROM ${schemaName}.products_grade_rows WHERE version_code = $1`,
          [versionCode]
        );
        ...
        for (const row of rows) {
          await client.query(`
            INSERT INTO ${schemaName}.products_grade_rows (
              version_code, class_code, class_name,
              grade, discount,pmcode, remark,
              created_by, updated_by
            )
        ...
        await client.query(`
          UPDATE ${schemaName}.products_grade_versions
  ...
  // ══════════════════════════════════════════════
  // 5. 刪除版本（連同明細）
  // DELETE /api/products-grade/versions/:versionCode
  // ══════════════════════════════════════════════
  router.delete('/versions/:versionCode',
    authenticateToken,
    requirePermission('sales:price-level:delete'),
    async (req, res) => {
      const client = await pool.connect();
      try {
        await client.query('BEGIN');
        ...
        await client.query(
          `DELETE FROM ${schemaName}.products_grade_versions WHERE version_code = $1`,
          [versionCode]
        );
  ...
  // ══════════════════════════════════════════════
  // 6. 查詢當前生效版本的價格（供訂單使用）
  // GET /api/products-grade/active
  // GET /api/products-grade/active?class_code=TS0001
  // ══════════════════════════════════════════════
  router.get('/active',
    authenticateToken,
    requirePermission('sales:price-level:read'),
      ...
      try {
        ...
        let query = `
          SELECT
            ...
            v.start_date,
            v.end_date
          FROM ${schemaName}.products_grade_rows r
          JOIN ${schemaName}.products_grade_versions v ON r.version_code = v.version_code
          WHERE r.version_code = $1
        `;
        const params = [activeCode];

        if (product_code) {
          query += ` AND r.class_code = $2`;
          params.push(class_code);
        }

        query += ` ORDER BY r.class_code ASC, r.grade ASC`;
        ...
        console.error('❌ 查詢生效等級失敗:', error);
        res.status(500).json({ status: 'Error', message: '查詢生效等級失敗', error: error.message });
      }
    }
  );
